#!/usr/bin/env python3
"""Validate Shopify App Store app icon basics without external deps."""
...
def validate(path: Path) -> list[str]:
    ...
    if (width, height) != (EXPECTED_SIZE, EXPECTED_SIZE):
        errors.append(f"Expected {EXPECTED_SIZE}x{EXPECTED_SIZE}, got {width}x{height}")
    if width != height:
        errors.append(f"Expected square aspect ratio, got {width}:{height}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate Shopify App Store app icon dimensions and format."
    )
    parser.add_argument("image", type=Path, help="Path to PNG or JPEG icon")
